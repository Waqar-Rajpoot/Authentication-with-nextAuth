...
//       <h1 className="text-3xl text-center font-bold text-green-400 mb-10">Admin Dashboard</h1>
...
//                 <button onClick={() => updateUserRole(user._id, user.role === "admin" ? "user" : "admin")} className="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600 hover:cursor-pointer">
//                   {user.role === "admin" ? "Demote" : "Promote"}
//                 </button>
//                 <button onClick={() => deleteUser(user._id)} className="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600 hover:cursor-pointer">
//                   Delete
//                 </button>
//               </td>
//             </tr>
...
// Define a User type based on your expected structure
type User = {
  _id: string;
  username: string;
  email: string;
...
export default function AdminPage() {
  ...
  useEffect(() => {
    async function fetchUsers() {
      try {
        const response = await axios.get<{ users: User[] }>("/api/users/admin/getUsers");
        setUsers(response.data.users);
      } catch (error: any) {
        toast.error(error.response?.data?.error || "Failed to fetch users.");
      }
